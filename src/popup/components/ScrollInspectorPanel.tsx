    ...
    <div className="p-4 space-y-4">
      ...
      <div className="bg-gradient-to-r from-blue-50 to-purple-50 rounded-lg p-4 border border-blue-200">
        ...
        <div className="relative h-8 bg-white rounded-lg overflow-hidden border border-gray-200">
          ...
          {/* Timeline markers - simplified for now */}
          {filteredAnimations.slice(0, 10).map((anim, idx) => (
            <div
              key={anim.id}
              className="absolute top-1/2 -translate-y-1/2 w-2 h-2 bg-blue-600 rounded-full border-2 border-white"
              style={{ left: `${(idx / Math.min(filteredAnimations.length, 10)) * 100}%` }}
              title={`${getLibraryInfo(anim.library).name} - ${anim.element}`}
            />
          ))}
        </div>
      </div>
                ...
                <div className="px-3 pb-3 space-y-3 border-t border-gray-100 bg-gray-50">
                  ...
                  <div className="flex gap-2 pt-2">
                    <button
                      ...
                      className="flex-1 flex items-center justify-center gap-1 px-3 py-1.5 bg-blue-50 hover:bg-blue-100 text-blue-700 rounded text-xs font-medium transition-colors"
                    >
                    ...
                    <button
                      ...
                      className="flex-1 flex items-center justify-center gap-1 px-3 py-1.5 bg-green-50 hover:bg-green-100 text-green-700 rounded text-xs font-medium transition-colors"
                    >
